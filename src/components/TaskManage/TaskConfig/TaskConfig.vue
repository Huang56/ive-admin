        ...
        <row :gutter="16">
          <Col :md="4" :lg="3">
            <FormItem label="任务类型">
              <Select v-model.trim="searchForm.qcGroup" clearable placeholder="请选择">
                <Option
                  v-for="(item,$index) in selectList.t_qc_group"
          ...
          <Col :md="4" :lg="3" v-first>
            <FormItem :label-width="0">
              <Button type="primary" @click="Search(searchForm)">
                <i class="icon-search">查询</i>
              </Button>
              <Button @click="searchForm={}">
                <i class="icon-reset">重置</i>
              </Button>
            </FormItem>
          </Col>
        </row>
        ...
        <row :gutter="16">
          ...
          <Col :span="12">
            <FormItem label="可指派人" prop="person">
              <Input
                v-model.trim="modalForm.person"
                search
                @on-search="iptSearch('person')"
                placeholder="请输入"
          ...
          <Col :span="12">
            <FormItem label="被指派组" prop="groups">
              <Input
                v-model.trim="modalForm.groups"
                search
                @on-search="iptSearchGroup"
                placeholder="请输入"
              ></Input>
            </FormItem>
          </Col>
          <Col :span="12">
            <FormItem label="允许转派否" prop="qcStandType">
              <Select v-model.trim="modalForm.qcStandType" placeholder="请选择">
                <Option
                  v-for="(item,$index) in selectList.qc_stand_type"
                  :value="item.itemValue"
                  :key="$index"
                >{{item.itemName}}</Option>
              </Select>
          ...
          <Col :span="12">
            <FormItem label="可转派人" prop="redeploy">
              <Input
                v-model.trim="modalForm.redeploy"
                search
                @on-search="iptSearch('redeploy')"
                placeholder="请输入"
              ></Input>
            </FormItem>
          </Col>
    ...
    >
      ...
      >
        <row :gutter="16">
          ...
          <Col :span="8">
            <FormItem label="人员编号">
              <Input
                v-model.trim="designateForm.qcStandNo"
                placeholder="请输入"
                :disabled="codeDisabled"
              ></Input>
            </FormItem>
      ...
      <div class="container">
        ...
        <canvasTable
          ...
          :tData="designateTD"
          :loading="tableLoading"
          @action="handleTablePerson"
          v-show="designateTitle==='请选择指派人员'"
        />

        <canvasTable
          :height="360"
          :tHeader="designateGroupTH"
          :tData="designateGroupTD"
          :loading="tableLoading"
          @action="handleTableGroup"
          v-show="designateTitle==='请选择工作组'"
        />
        ...
        <canvasTable
          ...
          :tData="redeployTD"
          :loading="tableLoading"
          @action="handleTableRedeploy"
          v-show="designateTitle==='请选择转派人员'"
        />
        <pageing
          :current="pageData.current"
          :total="pageData.total"
...
import { data as GetListData } from "./mockGetListData.js";
export default {
  name: "taskConfig",
  data() {
    return {
  ...
  methods: {
    Search(params) {
      ...
      let timer;
      let mockGetListQC = new Promise((resolve, reject) => {
        setTimeout(() => {
          resolve(GetListData);
        }, 3000);
      });
      ...
      mockGetListQC
        .then(resData => {
          ...
          this.tData = resData;
          this.designateTD = resData;
          this.designateGroupTD = resData;
          this.redeployTD = resData;
          this.pageData = { current: 2, total: 1000, size: 50 };
        })
        .catch(err => {
          alert("获取数据错误~");
        });
