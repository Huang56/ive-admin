...
export default {
  ...
  methods: {
    ...
    initScroll () {
      // 横向滚动条
      const { allRowData, allColData, box, canvas, rowH, ratio, flatData, cHeight, hScrollbar, vertical, vScrollbar } = this
      if (!canvas) {
        return
      }
      let cw = canvas.width
      let ch = canvas.height
      let ah = headerHeight * ratio + allRowData.length * rowH // 行总高度
      let aw = 0 // 列总长度
      vScrollbar.style.top = '0px'
      allColData.forEach(v => aw += v.width)
      if (aw <= cw) {
        this.showHScrollbar = false
        // this.allColData[allColData.length - 1].width += (cw - aw)
        aw = cw
        this.scroll.disx = 0
      } else {
      ...
      if (flatData.length === 0) {
        ...
        boxHeight = cHeight
        vScrollbar.style.height = (ch * ch / ah / ratio) + 'px'
      }
      vertical.style.height = (boxHeight - 32) + 'px'

      this.setCanvas(boxHeight)
    ...
    vScrollbarMouseUp (e) {
      ...
      this.scroll.vScrolling = (this.scroll.disy > 0)
      this.isCanvasMove = true
      this.isScrolling = false
      this.paint()
    },
    // 鼠标滚轮滚动事件
    vMouseWheel (e, data) {
      const { scroll, canvas, isScroll, horizontal, hScrollbar, showHScrollbar, showVScrollbar, vertical, vScrollbar } = this
      ...
      if (isHScroll && showHScrollbar) {
      ...
      } else if (!isHScroll && isScroll && showVScrollbar) {
        // 垂直溢出高度
        ...
        const scrollYRender = (isScroll = true) => {
          if (isScroll) {
            this.setTooltipPosition()
            this.resetEditCell()
            vScrollbar.style.top = (disy / overHeight) * disH + 'px'
            scroll.disy = disy
            scroll.sdisy = 0
            this.getDisplayRow()
            this.handleShowText()
