  ...
  methods: {
    ...
    integrateData() {
      ...
      if (fixedRight) {
        ...
        cols.push({...OPERATION_COL, width: operation.width, label: operation.label})
      }
      this.excludeSortIndex = sindex
      // 排除 show 为 false 的列
      column.forEach(v => {
        if (v.show) {
          cols.push({...v, width: parseInt(v.width)})
        }
      })
      ...
      for (let i = 0; i < alen; i++) {
        ...
        if (sort && sort.prop === key) {
          ...
          if (!this.sortCol.col.key) {
            ...
            this.sortCol.num = sortOrder[sort.order || sortOrder.asc||sortOrder.normal]
          }
        }
      }
      ...
      const res = data.map((item, index) => {
        ...
        for (let i = sindex; i < alen; i++) {
          ...
          if (col.formatter && val && typeof col.formatter === 'function') {
    ...
    setButtonDisable(item, index) {
      const {buttonOptions, operation} = this
      ...
      //   }
      // }
      operation.options.forEach(element => {
         // 权限控制
         let disabled = element.disabled
         if (this.buttonDisabledRows[index]) {
           this.buttonDisabledRows[index][element.type] = disabled
         } else {
           this.buttonDisabledRows[index] = {}
           this.buttonDisabledRows[index][element.type] = disabled
         }
    ...
    // 文字溢出处理
    handleShowText() {
      const {displayCol, displayRow, flatData, ratio, fixedLeftCol} = this
      let arr = []
      let voarr = []
      ...
      if (displayRow.length > 0) {
        displayRow.forEach(item => {
          if (item && typeof item.index === 'number') {
            ...
            colArr.forEach(obj => {
              // 处理溢出隐藏数据
              const {over, text} = textOverflow(this.textCtx, flatData[item.index][obj.key], obj.width, ratio)
              res[obj.key] = text
              vo[obj.key] = over
            })
